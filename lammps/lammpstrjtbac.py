...
#-----------------------------------------------------------
def writetbacxyz(ii,pos,box,v,jj,Ntypes):
    if os.path.isfile("tbacnorm.xyz"):
        f = open("tbacnorm.xyz","a")
    else:
        f = open("tbacnorm.xyz","w")
        f.write(str(Ntypes*53)+"\n")
        f.write("# tbac reference structures " + str(Ntypes) + "\n")
    ...
    atype = ["N",
            ...
            "C","C","C","C","H","H","H","H","H","H","H","H","H"]
    for i in range(53):
        dpos = pos[ii+i]-pos[ii]
        dpos -= numpy.floor(dpos/box+.5)*box # periodic boundary
        x = numpy.dot(dpos,v[0])
        y = numpy.dot(dpos,v[1])
        z = numpy.dot(dpos,v[2])
        f.write(atype[i]+" "+str(x)+" "+str(y)+" "+str(z)+"\n")
...
while(readconfig(f,natoms,box,pos,atypes,nconfig)):
    ...
    for i in range(nnitro):
        ...
        writetbacxyz(ii,pos,box,v,i,nnitro)
